...
const useVoiceRecognition = ({ onResult, isEnabled, requiredTier }) => {
    const [isListening, setIsListening] = useState(false);
    const recognitionRef = useRef(null);
    useEffect(() => {
        const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    ...
    const startListening = () => {
        if (!isEnabled) { alert(`جستجوی صوتی فقط برای کاربران ${requiredTier} و بالاتر فعال است.`); return; }
...
function App() {
    ...
    const handleSearch = async (queryToSearch) => {
        ...
        if (user?.subscriptionTier === 'free' && user.searchesLeft <= 0) return setShowUpgradeModal(true);
    ...
    const handleActorClick = (actorName) => { closeModal(); setTimeout(() => handleSearch(actorName), 300); };
    const handleMovieClick = async (movie) => {
        if (!user) return setShowLoginPromptModal(true);
        if (user?.subscriptionTier === 'free' && user.searchesLeft <= 0) return setShowUpgradeModal(true);
        ...
        try {
            ...
            closeModal();
            if (error.response?.status === 403) setError("محتوای این فیلم برای نمایش در دسترس نیست.");
            else setError('خطا در دریافت جزئیات.');
        } finally { setIsModalLoading(false); }
    };

    const isCinephileOrHigher = user && (user.subscriptionTier === 'cinephile' || user.subscriptionTier === 'directors_cut');

    const { isListening, startListening } = useVoiceRecognition({
        onResult: handleSearch,
        isEnabled: isCinephileOrHigher,
        requiredTier: 'Cinephile'
    });
                ...
                <div className="auth-section">
                    {user ? (
                        <div className="user-profile">
                            <span className="search-credit" title={user.subscriptionTier}>
                                {user.subscriptionTier === 'free' ? `${user.searchesLeft} جستجو` : user.subscriptionTier.replace('_', ' ')}
                            </span>
            ...
            <main>
                <form onSubmit={(e) => { e.preventDefault(); handleSearch(searchQuery); }}>
                    <div className="search-form-container">
                        ...
                        <button
                            type="button"
                            className={`voice-btn ${isListening ? 'listening' : ''} ${!isCinephileOrHigher ? 'disabled' : ''}`}
                            onClick={startListening}
                            title="جستجوی صوتی (ویژه Cinephile و بالاتر)"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z" /><path d="M17 11h-1c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92z" /></svg>
                        </button>
                ...
                {!loading && !error && movies.length > 0 && (
                    <section className="results-section">
                        <h2>{resultsTitle}</h2>
                        <div className="movies-grid">{movies.map(movie => <MovieCard key={`${movie.id}-${movie.credit_id || ''}`} movie={movie} onMovieClick={handleMovieClick} />)}</div>
                    </section>
                )}
            ...
            <div className="subscription-tiers">
                <div className="tier">
                    ...
                    <ul className="tier-features">
                        ...
                        <li>✓ **جستجوی صوتی هوشمند**</li>
                        <li>✓ دسترسی به تمام جزئیات</li>
                    </ul>
                    <button className="purchase-btn">انتخاب پلن Cinephile</button>
                </div>
                <div className="tier popular">
                    ...
                    <ul className="tier-features">
                        ...
                        <li>✓ **جستجو بر اساس آهنگساز**</li>
                        <li>✓ **جستجو بر اساس نویسنده**</li>
                    </ul>
                    <button className="purchase-btn">انتخاب Director's Cut</button>
                </div>
            </div>
        </div>
    </div>

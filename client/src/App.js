    ...
    useEffect(() => {
        ...
        recognition.onerror = (event) => {
            ...
            if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
                alert('دسترسی به میکروفون داده نشد. لطفاً در تنظیمات مرورگر خود دسترسی را فعال کنید.');
            }
            setIsListening(false);
        };
                ...
                <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="modal-header">
                        ...
                        <div className="modal-header-overlay">
                            <h2>{movie.title || movie.name}</h2>
                            <div className="modal-meta">
                                <span>{ (movie.release_date || movie.first_air_date) ? new Date(movie.release_date || movie.first_air_date).toLocaleDateString('fa-IR') : 'نامشخص'}</span>
                                <span>•</span>
                                <span>امتیاز: {Math.round(movie.vote_average * 10)} از ۱۰۰</span>
                            </div>
                        </div>
    ...
    return (
        <div className="App">
            ...
            <main>
                ...
                {/* این بخش اصلاح شد تا پیام "نتیجه‌ای یافت نشد" فقط زمانی نمایش داده شود که جستجویی انجام شده باشد */}
